@props([
    'iconButtonLabel' => '', // Default is Back
    'class' => ''
])
@php
    // color and icon
    $buttonStyles = [
        'Edit' => [
            'hover' => 'bg-blue-500',
            'icon' => "M9.195 18.44c1.25.714 2.805-.189 2.805-1.629v-2.34l6.945 3.968c1.25.715 2.805-.188 2.805-1.628V8.69c0-1.44-1.555-2.343-2.805-1.628L12 11.029v-2.34c0-1.44-1.555-2.343-2.805-1.628l-7.108 4.061c-1.26.72-1.26 2.536 0 3.256l7.108 4.061Z"
        ],
]
@endphp
<button {{$attributes}}
        class="relative group text-gray-500 transition-colors duration-200  hover:text-blue-600 focus:outline-none animate cursor-pointer">
    <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
          stroke-width="1.5" stroke="currentColor" class="w-5 h-5 ">
        <path stroke-linecap="round" stroke-linejoin="round"
              d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/>
    </svg>
    <div class="absolute invisible group-hover:visible -top-9 -right-2">
        <div class="bg-blue-600 text-white text-xs px-2 py-1 rounded-md">
            Edit
        </div>
        <div class="absolute left-[14px] w-0 h-0 border-l-[5px] border-l-transparent border-t-[7.5px] border-t-blue-600 border-r-[5px] border-r-transparent"></div>
    </div>
</button>
